...
// const PREFIX_FIELD = 3
// const SUFFIX_FIELD = 4
// const NATURE_FIELD = 5
const AS_IN_FIELD = 6
...
while ((line = liner.next())) {
  ...
  if (entry !== ent) {
    const vari = values[VARIATIONS_FIELD].split('/').map((x) => x.trim()).filter((x) => x.length > 0)
...
console.log('checked synonyms and confers')

// add aliases
const aliasArray = []
for (let index = 0; index < entries.length; index++) {
  const anEntry = entries[index]
  if (anEntry.docType === 'definition') 
    continue

  const alias_entries = anEntry.variations
  .filter((x) => x != anEntry.entry)

  alias_entries.forEach((x) => {
    if (aliasArray.findIndex((y) => y.docType=== 'entry'  && y.entry === x) != -1) 
      return

    if (entries.findIndex((y) => y.docType=== 'entry'  && y.entry === x) != -1) 
      return
    ...
    const alias = {
      ...
      docType: 'entry',
      // make sure entry is the first variations
      variations: anEntry.variations,
      aliasOf: anEntry.entry,
    }
    aliasArray.push(alias)
  })
}

entries.push(...aliasArray)
