...
personnel.push({ icon: 'bookmark', label: 'Mes lexiques', path: '/dashboard/lexicons', items: lexicons })

menus.push({ label: 'Général', items: general })
menus.push({ label: 'Personnel', items: personnel })

const removeByLabel = (label: string, items: DashboardMenuItem[]): DashboardMenuItem[] => {
...
export const useDicoStore = create<DicoStore>((set) => ({
	menus: menus,
