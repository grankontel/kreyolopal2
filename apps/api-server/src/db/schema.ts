import { pgTable, foreignKey, uuid, text, char, varchar, jsonb, timestamp, integer, index, unique, date, boolean, uniqueIndex, serial, primaryKey } from "drizzle-orm/pg-core"
...
export const auth_user = pgTable('auth_user', {
  ...
  (table) => {
    return {
      IX_auth_user_reset_pwd_token: index('IX_auth_user_reset_pwd_token').on(
        table.reset_pwd_token
      ),
      auth_user_username_key: unique('auth_user_username_key').on(
        table.username
      ),
...
export const permissions = pgTable('permissions', {
  ...
  (table) => {
    return {
      permissions_action_subject: unique('permissions_action_subject').on(
        table.action,
        table.subject
      ),
    }
...
export const users_roles = pgTable('users_roles', {
  ...
  (table) => {
    return {
      ...
      users_roles_user_id_role: primaryKey({
...
export const roles_permissions = pgTable( 'roles_permissions', {
  ...
  (table) => {
    return {
      roles_permissions_role_permission_id: primaryKey({
        columns: [table.permission_id, table.role],
...
export const spellcheckeds = pgTable('spellcheckeds', {
  ...
  export const ratings = pgTable('ratings', {
	spellchecked_id: uuid('spellchecked_id')
	  .primaryKey()
	  .notNull()
	  .references(() => spellcheckeds.id, { onDelete: 'cascade' }),
