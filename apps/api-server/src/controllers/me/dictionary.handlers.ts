...
import { WordsRepository } from '#services/word.repository'
...
const bookmarkWord = async function (c: Context) {
  ...
  if (!user) {
	...
	if (!enforcer.can('bookmark', 'dictionary')) {
		logger.debug('user does not have bookmark dictionary permission')
		return c.json({ error: 'Unsufficient permissions.' }, 403)
	}
...
const addConfer = async function (c: Context) {
  ...
  let { kreyol, rank, text }: { kreyol: string; rank: number; text: string } = c.req.valid('json')
