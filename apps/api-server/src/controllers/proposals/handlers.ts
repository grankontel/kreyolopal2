...
interface DatedEntry extends BaseEntry {
  creation_date: Date
}
...
const submitProposal = async function (c: Context) {
  ...
  try {
    ...
    let existingEntry = await coll.findOne<DatedEntry>(filter)
...
const listProposals = async function (c: Context) {
  ...
  try {
    ...
    const coll = mongo
      .db(config.mongodb.database)
      .collection(MongoCollection.proposals)
    const cursor = coll.aggregate(agg).skip(offset).limit(pagesize)
    const result = await cursor.toArray()
    cursor.close()
    ...
    if (result.length > 0) {
      ...
      c.status(200)
      return c.json<ProposalEntry[]>(result)
    }
