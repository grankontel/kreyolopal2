...
export const logger = (): MiddlewareHandler => {
    return async (c, next) => {
        ...
        const extra = { method: method, path: path, ip: parseIp(c.req), host: c.req.header('Host'), "user-agent": c.req.header('User-Agent') }
        ...
        winston_logger.info(`response : ${method} ${path} code: ${c.res.status} ${service}`, rextra);
